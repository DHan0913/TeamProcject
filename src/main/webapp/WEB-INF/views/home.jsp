	...
	<div id="container">
		...
		<c:if test="${authUser != null && authUser.role == 1}">
			<div class="text-center mb-4">
				<button class="btn btn-primary"
					onclick="location.href='${pageContext.request.contextPath}/admin/home'">관리자
					화면</button>
			</div>
		</c:if>
		...
		<div id="search-bar" class="container">
			<form id="searchForm"
				...
				<input type="hidden" name="filter" value="productName"> <input
		...
		<div id="content" class="container">
			<div class="button btn-group" role="group" aria-label="Basic example">
				...
				<button type="button" class="btn btn-secondary"
					onclick="window.location.href='${pageContext.request.contextPath}/'">필터해제</button>
				...
				<table class="table table-bordered">
					...
					<tbody>
						<c:forEach var="product" items="${products}">
							<tr>
								<td><h2>${product.productName}</h2><c:if test="${not empty product.img}">
										<a
											href="${pageContext.request.contextPath}/products/detail?productNo=${product.productNo}">
			...
			<div class="pagination">
				<c:if test="${currentPage > 1}">
					<a
						href="${pageContext.request.contextPath}/main?page=${currentPage - 1}"
						class="page-link">Previous</a>
				</c:if>
				<c:forEach var="i" begin="1" end="${totalPages}">
					<a href="${pageContext.request.contextPath}/main?page=${i}"
